...
populate_array data/raw/rnacentral-lncrna-seq.csv "lncrna"
populate_array data/raw/rnacentral-short-ncrna-seq.csv "short_ncrna"
populate_array data/raw/rnacentral-pre-mirna-seq.csv "pre_mirna"
...
for id in "${IDs_ncrna[@]}"; do
    ...
    if [[ "${IDs_lncrna[@]}" =~ "$id" ]]  
    then
        set_variables "$id" "data/raw/rnacentral-lncrna-seq.csv" 
        ...
        then 
            ...
            then 
                ...
                then
                    ...
                    echo RNA$lncrna_count,Yes,"$chr","$start_two","$end_two","$seq"     >> data/functional-lncrna-exon2-dataset.csv
                    echo RNA$lncrna_count,Yes,"$chr","$start_three","$end_three","$seq" >> data/functional-lncrna-exon3-dataset.csv  
    ...
    then
        set_variables "$id" data/raw/rnacentral-short-ncrna-seq.csv 
        if [ "$status" != 'not-pass' ]
        then
            ...
            then
                ...
                then
		            # To generate negative control sequences that are the same length as exons two and three
                    echo $chr,$end,$start,$len,'NA' >> data/coords-for-negative-control-ncrna.csv
                else
                    echo $chr,$start,$end,$len,'NA' >> data/coords-for-negative-control-ncrna.csv
                fi
            else
                :
            fi 
    ...
    then
        set_variables "$id" data/raw/rnacentral-pre-mirna-seq.csv 
        if [ "$status" != 'not-pass' ]
        then
            ...
            then
                (( pre_mirna_count++ ))
                start=$((zero_start+=1)) 
                echo RNA$pre_mirna_count,Yes,"$chr","$start","$end","$seq" >> data/functional-pre-mirna-dataset.csv
