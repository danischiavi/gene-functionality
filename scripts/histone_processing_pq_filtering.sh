...
if [ $# -ne 4 ]; then
    echo "Usage: $0 regions_to_extract.csv threshold histone_name outname"
    exit 1
fi
...
# AvgSignal = sum(wi * ei)
# Calculate the weighted sum of enrichment, grouped by Chromosome, StartExon, EndExon and ID
awk -F '\t' '
    BEGIN { OFS="\t" }
    {
        key = $1 FS $8 FS $9;
