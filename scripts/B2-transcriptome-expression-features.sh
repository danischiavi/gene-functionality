...
#
# Description: This script calculates the average number of reads per kilobase of transcripts per million mapped reads (RPKM) for protein-coding exons, lncRNA exons, short ncRNAs 
...
run_encode() {
    ...
    echo "RPKM_$sample_type" > "$output_file"
    ...
    tail -n +2 "$initial_data"  | while IFS=',' read -r _ _ chr start end _; do
        ...
        ######## Record RPKM across all RNAseq files
        count_max=0                                                                                  # To record maximum RPKM
        ...
        for file in $encode_data; do                                                                 # Loop through all RNAseq datasets
            ...
            if (( $(echo "$rpkm > $count_max" | bc -l) )) ; then count_max=$rpkm ; fi
        
        done       
                                                                                            
        if [ -z "$count_max" ] ; then count_max=NA ; fi                               # But if no transcription whatsoever, set everything to NA 

        echo "$count_max" >> "$output_file"
